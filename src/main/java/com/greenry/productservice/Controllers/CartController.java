...
public class CartController {
    ...
    public ResponseEntity<Cart> getUserCart(@PathVariable("userName")String userName){
        if((userName==null) || userService.checkValidity(userName)==false){
    ...
    @DeleteMapping("/{ptitle}/delete-product-from-cart")
    public ResponseEntity<Cart> deleteProduct(@PathVariable("userName")String userName,@PathVariable("ptitle")String ptitle){
        if((userName==null || ptitle==null) || userService.checkValidity(userName)==false){
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).build();
            ...
            if (user == null || product == null) {
                ...
                Cart cart = cartService.deleteProduct(user,product);
